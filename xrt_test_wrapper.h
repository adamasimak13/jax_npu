...
struct args parse_args(int argc, const char *argv[]) {
  ...
  options.add_options()
    ("input-file", "Input data file", cxxopts::value<std::string>()->default_value("input_data.bin"))
    ("output-file", "Output data file", cxxopts::value<std::string>()->default_value("output_data.bin"));
...
int setup_and_run_aie(int IN1_VOLUME, int IN2_VOLUME, int OUT_VOLUME,
  ...
  // sync host to device memories
    // Measure memory copy to device
  //auto start_memcpy_to_device = std::chrono::high_resolution_clock::now();
  ...
  //auto stop_memcpy_to_device = std::chrono::high_resolution_clock::now();
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    auto stop = std::chrono::high_resolution_clock::now();
      // Measure memory copy from device 
    auto start_memcpy_from_device = std::chrono::high_resolution_clock::now();
    bo_out.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
    if (myargs.trace_size > 0)
      bo_trace.sync(XCL_BO_SYNC_BO_FROM_DEVICE);
    auto stop_memcpy_from_device = std::chrono::high_resolution_clock::now();
    ...
    float memcpy_to_time = std::chrono::duration_cast<std::chrono::microseconds>(stop_memcpy_to_device - start_memcpy_to_device).count();
    float kernel_time = std::chrono::duration_cast<std::chrono::microseconds>(stop - start).count();
    float memcpy_from_time = std::chrono::duration_cast<std::chrono::microseconds>(stop_memcpy_from_device - start_memcpy_from_device).count();
    float total_time = memcpy_to_time + kernel_time + memcpy_from_time;

     //if (myargs.verbosity >= 1) {
      //std::cout << "Memcpy TO device: " << memcpy_to_time << "us\n";
      //std::cout << "Kernel execution: " << kernel_time << "us\n";
      //std::cout << "Memcpy FROM device: " << memcpy_from_time << "us\n";
...
int setup_and_run_aie(
    int IN1_VOLUME,
    int OUT_VOLUME,
    struct args myargs,
    std::function<void(T1*, int)> init_bufIn1,
    std::function<void(T3*, int)> init_bufOut,
    std::function<int(T1*, T3*, int, int)> verify_results
) {
  ...
  float npu_time_min = 9999999;
  float npu_time_max = 0;
  float kernel_time_total = 0;
  float memcpy_to_time_total = 0;
  float memcpy_from_time_total = 0;
  ...
  for (unsigned iter = 0; iter < num_iter; iter++) {
    ...
    //float npu_time =
      ...
      memcpy_to_time_total += memcpy_to_time;
      kernel_time_total += kernel_time;
      memcpy_from_time_total += memcpy_from_time;
      npu_time_total += total_time;
    ...
    unsigned valid_iters = myargs.n_iterations;
    std::cout << std::endl
          << "Avg Memcpy TO device: " << memcpy_to_time_total / valid_iters << "us" << std::endl
          << "Avg Kernel execution: " << kernel_time_total / valid_iters << "us" << std::endl
          << "Avg Memcpy FROM device: " << memcpy_from_time_total / valid_iters << "us" << std::endl
          << "Avg Total time: " << npu_time_total / valid_iters << "us" << std::endl;
